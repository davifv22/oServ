...
def update_empresa(empresa):
    empresa_model.EmpresaModel.query.filter_by(idEmpresa=1).update(
            {"nomeEmpresa": empresa['nomeEmpresa'], "dtRefSistema": empresa['dtRefSistema'], "dtImplantacao": empresa['dtImplantacao'], "endereco": empresa['endereco'], "cnpj": empresa['cnpj'], "cidade": empresa['cidade']})
